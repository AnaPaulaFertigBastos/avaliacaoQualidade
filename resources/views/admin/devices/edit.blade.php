  ...
  <div class="card">
    ...
    <form method="POST" action="{{ route('admin.devices.update', $device->id) }}">
      ...
      <div class="form-row" style="margin-bottom:12px;">
        ...
        <input type="text" id="nome" name="nome" value="{{ old('nome', $device->nome) }}" required style="width:100%;padding:8px;box-sizing:border-box;">
      </div>
      <div class="form-row" style="margin-bottom:12px;">
        <label><input type="checkbox" name="status" value="1" {{ old('status', $device->status) ? 'checked' : '' }}> Ativo</label>
      </div>
